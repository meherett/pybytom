...
from pybytom.wallet.tools import (
    get_program, get_address, get_vapor_address
)
...
p2wpkh_address = get_p2wpkh_address(public_key_hash=public_key_hash, network=NETWORK)
...
p2wpkh_vapor_address = get_p2wpkh_vapor_address(public_key_hash=public_key_hash, network=NETWORK)
assert get_vapor_address(program=get_program(public_key=PUBLIC_KEY), network=NETWORK) == p2wpkh_vapor_address
